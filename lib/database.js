...
module.exports = {
  ...
  get_addresses_balances: function(cb) {
    Address.find({ balance: { $gt: 0 } }, 'a_id balance').sort({balance: 'desc'}).exec(function(err, addresses){
      return cb({ 
        balances: addresses.map(function(address) { 
          return {
            address: address.a_id,
            balance: address.balance,
          }; 
        }),
        total: addresses.reduce(function(acc, address) {
          return { balance: acc.balance + address.balance };
        }).balance,
      });
    });
  },
